...
const InsuranceModal = ({
  ...
  useEffect(() => {
    if (mode === "edit" && editId) {
      setLoading(true);
      axios
        .get(`${InsuranceHistory()}/show/${editId}`, {
          headers: {
    ...
    const fetchExistingRecords = async () => {
      try {
        const humanResourceId = new URLSearchParams(window.location.search).get(
          "param"
        );
        const response = await axios.get(
          `${InsuranceHistory()}/${humanResourceId}`,
          {
            headers: {
              Authorization: `Bearer ${window.localStorage.getItem("token")}`,
            },
        ...
        setExistingRecords(
          response.data.filter((record) => record.id !== editId)
        );
      } catch (error) {
  ...
  const checkDateOverlap = (newStartDate, newEndDate) => {
    ...
    for (const record of sortedRecords) {
      const existingStartTimestamp = convertPersianDateToTimestamp(
        record.start_date
      );
      const existingEndTimestamp = convertPersianDateToTimestamp(
        record.end_date
      );

      // بررسی تداخل
      const isOverlapping =
        (newStartTimestamp >= existingStartTimestamp &&
          newStartTimestamp <= existingEndTimestamp) || // تاریخ شروع جدید در بازه موجود
        (newEndTimestamp >= existingStartTimestamp &&
          newEndTimestamp <= existingEndTimestamp) || // تاریخ پایان جدید در بازه موجود
        (newStartTimestamp <= existingStartTimestamp &&
          newEndTimestamp >= existingEndTimestamp); // بازه جدید کاملاً پوشش‌دهنده بازه موجود
  ...
  const handleSubmit = async (formData) => {
    formData.human_resource_nid = new URLSearchParams(
    ...
    try {
      ...
      const { hasOverlap, message } = checkDateOverlap(
        formData.start_date,
        formData.end_date
      );
      if (hasOverlap) {
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <DatePicker
                        ...
                        onChange={(date) =>
                          field.onChange(date ? date.toUnix() : null)
                        }
                        calendar={persian}
                        locale={persian_fa}
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      helperText={
                        methods.formState.errors.dehyari_title?.message
                      }
                    />
                  )}
                />
