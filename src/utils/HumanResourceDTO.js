...
function getVillageName(humanResourceData) {
    const { job_type_id, covered_villages, signature_data } = humanResourceData;
    if (job_type_id === 1) {
        return covered_villages[0]?.village?.approved_name || "";
    } else if (job_type_id === 3 || job_type_id === 4) {
        return signature_data?.village_employer?.approved_name || "";
    }
    return "";
}

function getContractRole(humanResourceData) {
    const { job_type_id } = humanResourceData;
    if (job_type_id === 1) {
        return {
            contractClause2: "موارد خاتمه کار دهیار همان است که در ماده ۵۲ آیین نامه اجرایی تشکیلات، انتخابات داخلی و امور مالی شوراهای اسلامی روستا و نحوه انتخاب دهیار مصوب ۱۳۷۸ و اصلاحات بعدی آن پیش بینی شده است.",
            contractClause3: "مدت مرخصی استحقاقی و مرخصی استعلاجی بر اساس ضوابط و مقررات قانون کار و قانون تامین اجتماعی می‌باشد.",
            signNote: `امضای ذیل این قرارداد از سوی بخشدار صرفا جهت اجرای ماده ۱۶ اساسنامه، تشکیلات و سازمان دهیاری‌ها مصوب ۱۳۸۰ و امضای مسئول امور مالی دهیاری به استناد ماده ۱۱ آیین نامه استخدامی دهیاری‌های کشور مصوب ۱۳۸۳ می‌باشد. کارفرما در این قرارداد دهیاری ${getVillageName(humanResourceData)} است`
        };
    } else if (job_type_id === 3 || job_type_id === 4) {
        return {
            contractClause2: "موارد خاتمه کار طرف قرارداد به استناد ماده ۲۱ قانون کار می‌باشد.",
            contractClause3: "ماموریت و مرخصی امور مالی دهیاری به استناد اصلاحیه ماده ۱۲ آئین نامه استخدامی دهیاری‌های کشور با تایید بخشدار صورت می‌گیرد.",
            signNote: `امضای ذیل این قرارداد از سوی بخشدار صرفا جهت اجرای ماده ۱۶ اساسنامه، تسهیلات و سازمان دهیاری‌ها مصوب ۱۳۸۰ و امضای مسئول امور مالی دهیاری به استناد ماده ۱۱ آﺋین نامه استخدامی دهیاری‌های کشور می‌باشد. دهیاری ${getVillageName(humanResourceData)} به نمایندگی از دهیاری‌های بند ۲ این قرارداد به عنوان دهیاری کارفرما تعیین می‌گردد`
        };
    }
    return {};
}
...
class HumanResourceDTO {
    constructor(humanResourceData) {
        try {
            ...
            this.marriedBenefits = formatCurrency(humanResourceData.salary?.married_benefits || 0);
            this.supervisorBenefits = formatCurrency(humanResourceData.salary?.supervisor_benefits || 0);
            ...
            const contractRole = getContractRole(humanResourceData);
            this.contractClause2 = contractRole.contractClause2 || '';
            this.contractClause3 = contractRole.contractClause3 || '';
            ...
            this.commitment3 = "عقد قرارداد هیچ‌گونه تعهدی مبنی بر استخدام اعم از رسمی یا پیمانی از سوی دهیاری برای طرف قرارداد ایجاد نمی‌کند";
            ...
            this.signingNote = contractRole.signNote || '';
            ...
            console.error("Error in HumanResourceDTO constructor:", error);
        }
    }
